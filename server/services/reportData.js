...
export default class ReportDataService {
  ...
  async postIntakeReport(data) {
    ...
    const reportData = new ReportData();
    const result = await reportData.postIntakeReport(data);
    return result;
  }
  ...
  async postDischargeReport(data) {
    ...
    const schema = joi.object({
      ...
      discharge_reason_other: joi.string().optional(),
      treatment_summary: joi.string().optional(),
      remaining_concerns: joi.string().optional(),
      recommendations: joi.string().optional(),
      client_understanding: joi.string().optional(),
      assessments: joi.array().items(joi.object({
        form_name: joi.string().optional(),
        score: joi.alternatives().try(joi.string(), joi.number()).optional(),
        therapist_notes: joi.string().optional(),
